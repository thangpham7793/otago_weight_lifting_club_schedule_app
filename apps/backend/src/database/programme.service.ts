import { hash } from "../utils/cryptoService.ts";
import { Request, Response, TimeTable } from "../types.d.ts";
...
export async function getAllSchedules(req: Request, res: Response) {
  ...
  const schedules = rows.map((schedule: any) => {
...
export async function createWeeklySchedules(req: Request, res: Response) {
  ...
  if (programmeIds.length > 0) {
    const tasks = programmeIds.map((programmeId: number) => {
...
export async function publishSchedule(req: Request, res: Response) {
  ...
  console.log(
    `Publish schedule ${scheduleId} to programmes ${
      JSON.stringify(
        programmeIds,
      )
    }`,
...
export async function getAllExercises(req: Request, res: Response) {
  ...
  const { rows } = await execute(statement);
  const exerciseNames = rows.reduce(
    (acc: any, exercise: { exerciseName: any }) => {
