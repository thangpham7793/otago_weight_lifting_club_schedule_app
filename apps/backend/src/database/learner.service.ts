import { compare } from "../utils/cryptoService.ts";
import { HttpError, makeToken } from "./../utils/index.ts";
import { NextFunction, Request, Response } from "../types.d.ts";
import { execute } from "./execute.ts";

export function redirectToSignupPage(_req: Request, res: Response) {
...
export async function createLearner(
  ...
  newLearnerInfo.username = `${newLearnerInfo.lastName}${
  ...
  const { username, learnerId } = rows[0];
...
export async function getPbs(req: Request, res: Response) {
